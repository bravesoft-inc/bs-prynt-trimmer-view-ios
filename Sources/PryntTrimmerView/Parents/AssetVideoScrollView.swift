...
protocol AssetVideoScrollViewDelegate: NSObject {
    func didDrawedView()
}

class AssetVideoScrollView: UIScrollView {
    weak var viewDelegate: AssetVideoScrollViewDelegate?
    ...
    override func draw(_ rect: CGRect) {
        ...
        print("AssetVideoScrollView")
        print("contentSize: \(contentSize)")
        print("view.frame: \(frame)")
        viewDelegate?.didDrawedView()
    }
    ...
    private func getThumbnailTimes(for asset: AVAsset, numberOfThumbnails: Int) -> [NSValue] {
        let timeIncrement = (asset.duration.seconds * 1_000) / Double(numberOfThumbnails)
        var timesForThumbnails = [NSValue]()
        for index in 0..<numberOfThumbnails {
            let cmTime = CMTime(value: Int64(timeIncrement * Float64(index)), timescale: 1_000)
        ...
        let handler: AVAssetImageGeneratorCompletionHandler = { [weak self] _, cgimage, _, result, error in
            if let cgimage = cgimage, error == nil && result == AVAssetImageGenerator.Result.succeeded {
                DispatchQueue.main.async(execute: { [weak self] () -> Void in
                    ...
                    self?.displayImage(cgimage, at: counting)
                    counting += 1
                })
            }
        }
